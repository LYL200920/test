...
namespace robot_model
{
  ...
  public:
    ...
    Point_Cloud_Collision_Snapshot(const Point_Cloud_Collision_Snapshot &) = delete;
    Point_Cloud_Collision_Snapshot &operator=(const Point_Cloud_Collision_Snapshot &) = delete;
  ...
  public:
    ...
    Robot_Collision_Detector &operator=(const Robot_Collision_Detector &) = delete;
    Robot_Collision_Detector(Robot_Collision_Detector &&) noexcept;
    Robot_Collision_Detector &operator=(Robot_Collision_Detector &&) noexcept;

    void Set_Robot_Parts(const std::vector<Robot_Visual_Part> &parts);
    void Set_Robot_Parts_For_Obstacle_Filtering(const std::vector<Robot_Visual_Part> &parts);
    void Set_Scene_Collision_Options(const Robot_Scene_Collision_Options &options);
    const Robot_Scene_Collision_Options &Scene_Collision_Options() const;
    bool Set_Obstacle_Points(const std::vector<float> &xyz, std::string *error_message = nullptr);
    bool Set_Obstacle_Points(const std::vector<float> &xyz,
    ...
    void Set_Obstacle_Snapshot(std::shared_ptr<const Point_Cloud_Collision_Snapshot> snapshot);
    ...
    Robot_Collision_Result Check_Pose(const std::vector<Matrix4> &world_from_parts, double clearance_mm) const;

  private:
    void Set_Robot_Parts_Internal(const std::vector<Robot_Visual_Part> &parts, bool build_pose_query_geometry);
    class Implementation;
    std::unique_ptr<Implementation> m_implementation;
  };
