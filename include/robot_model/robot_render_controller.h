...
namespace robot_model
{
  ...
  public:
    ...
    Robot_Joint_State_Apply_Result Try_Set_Joint_State(const Robot_Joint_State &joint_state);
    ...
    bool Set_Collision_Settings(const Robot_Collision_Settings &settings, std::string *error_message = nullptr);
    ...
    bool Create_Collision_Points_Rebuild_Request(std::shared_ptr<const std::vector<float>> xyz,
                                                 Collision_Index_Build_Request *request,
                                                 std::string *error_message = nullptr) const;
    bool Create_Collision_Settings_Rebuild_Request(const Robot_Collision_Settings &settings,
                                                   Collision_Index_Build_Request *request,
                                                   std::string *error_message = nullptr) const;
    void Apply_Collision_Rebuild_Result(Collision_Index_Build_Result result);
    Robot_Position_IK_Result Move_Flange_To(const Point3 &target_world);
    Robot_Pose_IK_Result Move_Flange_To_Pose(const Matrix4 &target_world_from_flange, const Robot_Pose_IK_Options &options = {});

  private:
    Robot_Joint_State_Apply_Result Try_Set_Joint_State_With_Refinement(const Robot_Joint_State &joint_state,
    ...
    bool Rebuild_Collision_Obstacle_Points(std::string *error_message = nullptr);
