...
public:
  ...
  bool Create_Points_Rebuild_Request (
    std::shared_ptr<const std::vector<float>> xyz,
    const Robot_Joint_State& reference_joint_state,
    const std::vector<Matrix4>& reference_world_from_parts,
    Collision_Index_Build_Request* request,
    std::string* error_message = nullptr) const;
  bool Create_Settings_Rebuild_Request (
    const Robot_Collision_Settings& settings,
    const std::vector<Matrix4>& reference_world_from_parts,
    Collision_Index_Build_Request* request,
    std::string* error_message = nullptr) const;
...
private:
  static bool Validate_Settings (
    const Robot_Collision_Settings& settings,
    std::string* error_message);
  bool Rebuild_Obstacle_Points (
    const std::vector<Matrix4>& reference_world_from_parts,
    std::string* error_message);
