...
std::uint64_t Camera_Service::Request_Apply_Parameters(std::vector<Camera_Parameter_Update> updates)
{
  return Enqueue(Camera_Operation::Apply_Parameters, {}, std::move(updates));
}
...
std::uint64_t Camera_Service::Enqueue(Camera_Operation operation,
                                      ...
                                      std::vector<Camera_Parameter_Update> parameter_updates)
{
  const auto request_id = m_next_request_id.fetch_add(1);
  {
    ...
    m_commands.push_back({request_id,
                          operation,
