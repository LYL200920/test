...
namespace
{
  ...
  const Value *Get_Value(const Camera_Parameter_Value &source,
                         const char *expected,
                         std::string *error)
  {
    ...
    if (value == nullptr)
    {
      Return_Error(std::string("Camera parameter value is not ") + expected, error);
    }
    return value;
  }
} // namespace
                             ...
                             std::string *error)
{
  ...
  case ParamType_Enum:
  {
    ...
    const auto count = std::min<std::size_t>(source.ParamInfo.stEnumParam.nSupportedNum, MV3D_RGBD_MAX_ENUM_COUNT);
    parameter->enum_values.assign(source.ParamInfo.stEnumParam.nSupportValue, source.ParamInfo.stEnumParam.nSupportValue + count);
    return true;
  }

  case ParamType_String:
    parameter->type = Camera_Parameter_Type::String;
    parameter->current_value = From_Fixed_String(source.ParamInfo.stStringParam.chCurValue);
    parameter->string_max_length = std::min<std::uint32_t>(source.ParamInfo.stStringParam.nMaxLength, MV3D_RGBD_MAX_STRING_LENGTH - 1);
    return true;
                                    ...
                                    std::string *error)
{
  ...
  *destination = {};
  ...
  case Camera_Parameter_Type::String:
  {
    ...
    const auto copy_length = std::min(value->size(), sizeof(destination->ParamInfo.stStringParam.chCurValue) - 1);
    std::memcpy(destination->ParamInfo.stStringParam.chCurValue, value->data(), copy_length);
                                      ...
                                      std::string *error)
{
  ...
  case Camera_Parameter_Type::Int:
  {
    ...
    if (current.int_increment && *current.int_increment > 0 && current.int_minimum && (*value - *current.int_minimum) % *current.int_increment != 0)
  ...
  case Camera_Parameter_Type::Enum:
  {
    ...
    if (!current.enum_values.empty() &&
        std::find(current.enum_values.begin(), current.enum_values.end(), *value) == current.enum_values.end())
