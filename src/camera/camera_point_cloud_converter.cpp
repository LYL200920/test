  ...
  std::uint32_t required_coordinate_type)
{
  ...
  for( const auto& image : frame.images )
  {
    ...
    if( required_coordinate_type != 0 &&
        image.coordinate_type != required_coordinate_type )
    {
      continue;
    }
  ...
  std::uint32_t required_coordinate_type)
{
  ...
  *destination = {};
  ...
  Point_Cloud_Format format;
  const auto* image = Find_Point_Cloud_Image (
    frame, &format, required_coordinate_type);
  if( image == nullptr )
  {
    if( error )
    {
      *error = required_coordinate_type == 0
        ? "Current frame does not contain point-cloud data"
  ...
  std::string* error)
{
  ...
  if( cloud == nullptr || cloud->xyz.empty ( ) ||
      ( cloud->xyz.size ( ) % 3 ) != 0 )
  {
    if( error ) *error = "Point cloud is empty or malformed";
    return false;
  }
  const std::size_t input_count = cloud->Point_Count ( );
  if( !cloud->rgb.empty ( ) && cloud->rgb.size ( ) != input_count * 3 )
  {
    if( error ) *error = "Point-cloud color array size does not match coordinates";
    return false;
  }
  ...
  {
    if( error ) *error = "Point-cloud overlay filter options are invalid";
    return false;
  }
  ...
  {
    const auto raw_min_max = std::minmax_element (
      raw_positive_depths.begin ( ), raw_positive_depths.end ( ));
    raw_depth_min = *raw_min_max.first;
    raw_depth_max = *raw_min_max.second;
    const auto raw_middle = raw_positive_depths.begin ( ) +
      raw_positive_depths.size ( ) / 2;
    std::nth_element (
      raw_positive_depths.begin ( ), raw_middle,
  ...
  const std::size_t output_count = std::min (
    valid_indices.size ( ), options.maximum_points);
  std::vector<float> filtered_xyz;
  std::vector<std::uint8_t> filtered_rgb;
  filtered_xyz.reserve (output_count * 3);
  if( cloud->Has_Color ( ) ) filtered_rgb.reserve (output_count * 3);
  for( std::size_t output_index = 0; output_index < output_count;
       ++output_index )
  {
    const std::size_t valid_position =
      output_index * valid_indices.size ( ) / output_count;
    const std::size_t source_index = valid_indices[valid_position];
    filtered_xyz.push_back (static_cast<float> (
      cloud->xyz[source_index * 3] * options.coordinate_to_mm));
    filtered_xyz.push_back (static_cast<float> (
      cloud->xyz[source_index * 3 + 1] * options.coordinate_to_mm));
    filtered_xyz.push_back (static_cast<float> (
      cloud->xyz[source_index * 3 + 2] * options.coordinate_to_mm));
    if( cloud->Has_Color ( ) )
    {
      filtered_rgb.insert (filtered_rgb.end ( ), {
        cloud->rgb[source_index * 3], cloud->rgb[source_index * 3 + 1],
