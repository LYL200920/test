...
bool Pixel_Count (
  const Camera_Image_Frame& image,
...
const Camera_Image_Frame* Select_Image (
  ...
  Camera_Image_Display_Mode mode)
{
  const auto find_type = [&frame] (bool (*predicate) (std::uint32_t))
  {
    const auto found = std::find_if (
      frame.images.begin ( ), frame.images.end ( ),
  ...
  std::string* error)
{
  ...
  *destination = {};
  ...
  return Return_Error (
    "Image type is not supported by the preview converter", error);
}
