...
Camera_Config_Dialog::Camera_Config_Dialog(wxWindow *parent, Camera_Service &camera_service)
...
void Camera_Config_Dialog::Build_Ui()
{
  ...
  m_refresh_button = new wxButton(this, wxID_REFRESH, From_Utf8(u8"刷新设备"));
  m_ok_button = new wxButton(this, wxID_OK, From_Utf8(u8"打开"));
  auto *cancel_button = new wxButton(this, wxID_CANCEL, From_Utf8(u8"取消"));
  m_ok_button->SetDefault();
  m_ok_button->Enable(false);

  m_refresh_button->Bind(wxEVT_BUTTON, &Camera_Config_Dialog::On_Refresh, this);
  m_ok_button->Bind(wxEVT_BUTTON, &Camera_Config_Dialog::On_Confirm, this);
  m_device_list->Bind(wxEVT_LIST_ITEM_SELECTED, &Camera_Config_Dialog::On_Device_Selection_Changed, this);
  m_device_list->Bind(wxEVT_LIST_ITEM_DESELECTED, &Camera_Config_Dialog::On_Device_Selection_Changed, this);
  m_device_list->Bind(wxEVT_LIST_ITEM_ACTIVATED, &Camera_Config_Dialog::On_Device_Activated, this);
...
void Camera_Config_Dialog::Load_Devices()
{
  ...
  if (!m_camera_service.Can_Refresh_Devices())
  {
    ...
    m_status_text->SetLabel(m_camera_service.State() == Camera_State::Error ? From_Utf8(u8"相机操作失败：") + From_Utf8(m_camera_service.Last_Error())
  ...
  m_status_text->SetLabel(initializing ? From_Utf8(u8"正在初始化SDK并搜索设备……")
                                       : From_Utf8(u8"正在搜索设备……"));
  Set_Busy(true);
  Layout();
  Update();
...
void Camera_Config_Dialog::Populate_Device_List()
{
  ...
  for (std::size_t i = 0; i < devices.size(); ++i)
  {
    ...
    const auto row = m_device_list->InsertItem(static_cast<long>(i), wxString::Format("%zu", device.index + 1));
    ...
    m_device_list->SetItem(row, 4, device.ip_address.empty() ? "-" : From_Utf8(device.ip_address));
    m_device_list->SetItemData(row, static_cast<wxUIntPtr>(i));

    if (!selected_serial.empty() && device.serial_number == selected_serial)
    {
      m_device_list->SetItemState(row, wxLIST_STATE_SELECTED | wxLIST_STATE_FOCUSED, wxLIST_STATE_SELECTED | wxLIST_STATE_FOCUSED);
...
{
  m_busy = busy;
  m_refresh_button->Enable(!busy && m_camera_service.Can_Refresh_Devices());
...
{
  m_state_text->SetLabel(From_Utf8(u8"状态：") + Camera_State_Label(m_camera_service.State()));
}

void Camera_Config_Dialog::Update_Selection_State()
{
  m_ok_button->Enable(!m_busy &&
...
{
  return m_device_list->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
}
...
void Camera_Config_Dialog::On_Confirm(wxCommandEvent &)
{
  ...
  const auto device_index = static_cast<std::size_t>(m_device_list->GetItemData(row));
  const auto &devices = m_camera_service.Devices();
  ...
  m_pending_request = m_camera_service.Request_Select_Device(devices[device_index].serial_number);
  if (m_pending_request == 0)
  {
    m_close_on_success = false;
...
void Camera_Config_Dialog::On_Camera_Updated(wxThreadEvent &event)
{
  ...
  const bool request_matches = m_pending_request == 0 || completed_request == m_pending_request;
  ...
  if (operation == Camera_Operation::Refresh_Devices && request_matches)
  {
    if (m_camera_service.Is_Initialized())
    {
      ...
      m_version_text->SetLabel(wxString::Format(From_Utf8(u8"SDK版本：%u.%u.%u"), version.major, version.minor, version.revision));
    }
    else
    {
    ...
    if (m_camera_service.Last_Operation_Succeeded())
    {
      ...
      m_status_text->SetLabel(device_count == 0 ? From_Utf8(u8"未发现3D相机，请检查连接后刷新")
                                                : wxString::Format(From_Utf8(u8"已发现 %zu 台设备"), device_count));
    }
    else
    {
      m_status_text->SetLabel(From_Utf8(u8"操作失败：") + From_Utf8(m_camera_service.Last_Error()));
    }
  }
  else if (operation == Camera_Operation::Select_Device && request_matches)
  {
    ...
    m_status_text->SetLabel(From_Utf8(u8"设备选择失败：") + From_Utf8(m_camera_service.Last_Error()));
  }
  else if (operation == Camera_Operation::Open_Device && request_matches)
  {
    ...
    m_status_text->SetLabel(From_Utf8(u8"相机打开失败：") + From_Utf8(m_camera_service.Last_Error()));
  }
  else if (operation == Camera_Operation::Frame_Error)
  {
    m_status_text->SetLabel(From_Utf8(u8"采集失败：") + From_Utf8(m_camera_service.Last_Error()));
  }
