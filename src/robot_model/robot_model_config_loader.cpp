...
namespace robot_model
{
  ...
  Robot_Kinematic_Params Load_Robot_Kinematic_Params(const std::filesystem::path &xml_path,
                                                     const std::string &default_name)
  {
    ...
    params.model_name = item_value_as_string(root, "ManipulatorModel", default_name.c_str());
    ...
    params.self_collision_clearance_mm = item_value_as_double(root, "SelfCollisionClearance", 3.0);
    params.self_collision_pairs = parse_part_pairs(item_value_as_string(root, "SelfCollisionPairs", ""));
    params.ground_collision_parts = parse_index_list(item_value_as_string(root, "GroundCollisionParts", ""));

    params.link_lengths.clear();
    for (int i = 1; i <= 6; ++i)
    {
      const auto key = std::string("LinkLength_") + std::to_string(i);
      params.link_lengths.push_back(item_value_as_double(root, key.c_str(), 0.0));
    }
    ...
    for (int i = 1; i <= 6; ++i)
    {
      ...
      const auto direction_key = std::string("JointDirection_") + std::to_string(i);
      ...
      params.joint_offsets.push_back(item_value_as_double(root, offset_key.c_str(), 0.0));
      params.joint_directions.push_back(item_value_as_int(root, direction_key.c_str(), 1));
      params.joint_mins.push_back(item_value_as_double(root, min_key.c_str(), -180.0));
      params.joint_maxs.push_back(item_value_as_double(root, max_key.c_str(), 180.0));
      ...
      joint_frame.has_pivot = item_exists(root, pivot_x_key.c_str()) &&
                              item_exists(root, pivot_y_key.c_str()) &&
                              item_exists(root, pivot_z_key.c_str());
      if (joint_frame.has_pivot)
      {
        joint_frame.pivot[0] = item_value_as_double(root, pivot_x_key.c_str(), 0.0);
        joint_frame.pivot[1] = item_value_as_double(root, pivot_y_key.c_str(), 0.0);
        joint_frame.pivot[2] = item_value_as_double(root, pivot_z_key.c_str(), 0.0);
      }

      joint_frame.has_axis = item_exists(root, axis_x_key.c_str()) &&
                             item_exists(root, axis_y_key.c_str()) &&
                             item_exists(root, axis_z_key.c_str());
      if (joint_frame.has_axis)
      {
        joint_frame.axis[0] = item_value_as_double(root, axis_x_key.c_str(), 0.0);
        joint_frame.axis[1] = item_value_as_double(root, axis_y_key.c_str(), 0.0);
        joint_frame.axis[2] = item_value_as_double(root, axis_z_key.c_str(), 1.0);
      }
    }
    ...
    {
      params.neutral_flange_pose[0] = item_value_as_double(root, "FlangePose_X", 0.0);
      params.neutral_flange_pose[1] = item_value_as_double(root, "FlangePose_Y", 0.0);
      params.neutral_flange_pose[2] = item_value_as_double(root, "FlangePose_Z", 0.0);
      params.neutral_flange_pose[3] = item_value_as_double(root, "FlangePose_A", 0.0);
      params.neutral_flange_pose[4] = item_value_as_double(root, "FlangePose_B", 0.0);
      params.neutral_flange_pose[5] = item_value_as_double(root, "FlangePose_C", 0.0);
    }

    for (int i = 0; i < 7; ++i)
    {
      const auto prefix = std::string("Part_") + std::to_string(i);
      params.manual_part_calibration[i].translate[0] = item_value_as_double(root, (prefix + "_OffsetX").c_str(), 0.0);
      params.manual_part_calibration[i].translate[1] = item_value_as_double(root, (prefix + "_OffsetY").c_str(), 0.0);
      params.manual_part_calibration[i].translate[2] = item_value_as_double(root, (prefix + "_OffsetZ").c_str(), 0.0);
      params.manual_part_calibration[i].rotate_xyz[0] = item_value_as_double(root, (prefix + "_RotX").c_str(), 0.0);
      params.manual_part_calibration[i].rotate_xyz[1] = item_value_as_double(root, (prefix + "_RotY").c_str(), 0.0);
      params.manual_part_calibration[i].rotate_xyz[2] = item_value_as_double(root, (prefix + "_RotZ").c_str(), 0.0);
    }
