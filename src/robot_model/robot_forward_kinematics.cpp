...
namespace robot_model
{
  namespace
  {
    ...
    Matrix4 rotation_about_point(double angle_deg,
                                 ...
                                 const Point3 &point)
    {
      ...
      matrix[2][2] = t * z * z + c;
      matrix[0][3] = point[0] - (matrix[0][0] * point[0] + matrix[0][1] * point[1] + matrix[0][2] * point[2]);
      matrix[1][3] = point[1] - (matrix[1][0] * point[0] + matrix[1][1] * point[1] + matrix[1][2] * point[2]);
      matrix[2][3] = point[2] - (matrix[2][0] * point[0] + matrix[2][1] * point[1] + matrix[2][2] * point[2]);
      return matrix;
    }

    Point3 part_origin_or(std::size_t part_count,
    ...
    Point3 configured_pivot_or(const Robot_Kinematic_Params &params,
                               std::size_t index,
                               const Point3 &fallback)
    {
    ...
    Point3 configured_axis_or(const Robot_Kinematic_Params &params,
                              std::size_t index,
                              const Point3 &fallback)
    {
      if (index >= params.joint_frames.size() ||
    ...
    {
      return index < state.delta_angles_deg.size() ? state.delta_angles_deg[index]
                                                   : 0.0;
    }
                                                                ...
                                                                const Robot_Kinematic_Params &params)
  {
    ...
    const auto a2 = part_origin_or(part_count, calibration, 2, shoulder_fallback);
    const auto a3 = part_origin_or(part_count, calibration, 3, elbow_fallback);
    const auto a4 = part_origin_or(part_count, calibration, 4, wrist_fallback);
    const auto a5 = part_origin_or(part_count, calibration, 5, a4);
    const auto a6 = part_origin_or(part_count, calibration, 6, a5);

    model.joint_pivots = {configured_pivot_or(params, 0, base_origin),
    ...
    model.joint_axes = {configured_axis_or(params, 0, {0.0, 0.0, 1.0}),
    ...
    {
      model.neutral_world_from_parts.push_back(Build_Part_Calibration_Matrix(calibration.parts[i]));
    }
    ...
    {
      const auto neutral_world_from_flange = Build_Zyx_Pose_Matrix(params.neutral_flange_pose);
      model.flange_from_last_part = Multiply_Matrices(Invert_Rigid_Matrix(model.neutral_world_from_parts.back()),
                                                             ...
                                                             const Robot_Joint_State &joint_state)
  {
    ...
    for (std::size_t i = 0; i < motion_after_joint.size(); ++i)
    {
      ...
      const auto axis_world = normalized_axis(transform_vector(chain, model.joint_axes[i]));
      result.joint_positions_world[i] = pivot_world;
      result.joint_axes_world[i] = axis_world;
      const auto joint_motion = rotation_about_point(joint_delta_at(joint_state, i), axis_world, pivot_world);
    ...
    {
      const Matrix4 motion = i == 0 ? identity_matrix()
                                    : motion_after_joint[std::min<std::size_t>(i, 6) - 1];
      result.world_from_parts.push_back(Multiply_Matrices(motion, model.neutral_world_from_parts[i]));
    }

    result.has_flange = model.has_flange && !result.world_from_parts.empty();
    if (result.has_flange)
    {
      result.world_from_flange = Multiply_Matrices(result.world_from_parts.back(), model.flange_from_last_part);
    }
    return result;
  }
