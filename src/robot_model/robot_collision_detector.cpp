...
namespace robot_model
{
  namespace
  {
    ...
    {
      return {
          static_cast<std::int64_t>(std::floor(point[0] / voxel_size_mm)),
          static_cast<std::int64_t>(std::floor(point[1] / voxel_size_mm)),
          static_cast<std::int64_t>(std::floor(point[2] / voxel_size_mm))};
    }
    ...
    std::array<double, 6> transformed_bounds(const std::array<double, 6> &local_bounds,
                                             const Matrix4 &world_from_part)
    {
      ...
      for (int x_side = 0; x_side < 2; ++x_side)
      {
        for (int y_side = 0; y_side < 2; ++y_side)
        {
          ...
          {
            const auto corner = Transform_Position(world_from_part, {local_bounds[x_side],
                                                                     local_bounds[2 + y_side],
                                                                     local_bounds[4 + z_side]});
            for (int axis = 0; axis < 3; ++axis)
            {
              result[axis * 2] = std::min(result[axis * 2], corner[axis]);
              result[axis * 2 + 1] = std::max(result[axis * 2 + 1], corner[axis]);
            }
          }
        }
      }
    ...
    bool point_inside_inflated_bounds(const Point3 &point,
    ...
    bool bounds_overlap(const std::array<double, 6> &first,
    ...
    bool oriented_bounds_overlap(const std::array<double, 6> &first_bounds,
                                 const Matrix4 &world_from_first,
                                 const std::array<double, 6> &second_bounds,
                                 const Matrix4 &world_from_second)
    {
      const Point3 first_center_local = {(first_bounds[0] + first_bounds[1]) * 0.5,
                                         (first_bounds[2] + first_bounds[3]) * 0.5,
                                         (first_bounds[4] + first_bounds[5]) * 0.5};
      const Point3 second_center_local = {(second_bounds[0] + second_bounds[1]) * 0.5,
                                          (second_bounds[2] + second_bounds[3]) * 0.5,
                                          (second_bounds[4] + second_bounds[5]) * 0.5};
      const auto first_center = Transform_Position(world_from_first, first_center_local);
      const auto second_center = Transform_Position(world_from_second, second_center_local);
      const std::array<double, 3> first_half = {(first_bounds[1] - first_bounds[0]) * 0.5,
                                                (first_bounds[3] - first_bounds[2]) * 0.5,
                                                (first_bounds[5] - first_bounds[4]) * 0.5};
      const std::array<double, 3> second_half = {(second_bounds[1] - second_bounds[0]) * 0.5,
      ...
      for (int axis = 0; axis < 3; ++axis)
      {
        first_axes[axis] = {world_from_first[0][axis],
                            world_from_first[1][axis],
                            world_from_first[2][axis]};
        second_axes[axis] = {world_from_second[0][axis],
                             world_from_second[1][axis],
      ...
      for (int first_axis = 0; first_axis < 3; ++first_axis)
      {
        ...
        {
          rotation[first_axis][second_axis] = dot(first_axes[first_axis], second_axes[second_axis]);
          absolute_rotation[first_axis][second_axis] = std::abs(rotation[first_axis][second_axis]) + 1.0e-9;
        }
      }
      const Point3 center_delta = {second_center[0] - first_center[0],
                                   second_center[1] - first_center[1],
                                   second_center[2] - first_center[2]};
      const std::array<double, 3> translation = {dot(center_delta, first_axes[0]),
                                                 dot(center_delta, first_axes[1]),
                                                 dot(center_delta, first_axes[2])};

      for (int axis = 0; axis < 3; ++axis)
      {
        const double second_radius = second_half[0] * absolute_rotation[axis][0] +
                                     second_half[1] * absolute_rotation[axis][1] +
                                     second_half[2] * absolute_rotation[axis][2];
        if (std::abs(translation[axis]) >
            first_half[axis] + second_radius)
          return false;
      }
      for (int axis = 0; axis < 3; ++axis)
      {
        const double first_radius = first_half[0] * absolute_rotation[0][axis] +
                                    first_half[1] * absolute_rotation[1][axis] +
                                    first_half[2] * absolute_rotation[2][axis];
        const double projected_translation = std::abs(translation[0] * rotation[0][axis] +
      ...
      for (int first_axis = 0; first_axis < 3; ++first_axis)
      {
        ...
        for (int second_axis = 0; second_axis < 3; ++second_axis)
        {
          ...
          const double first_radius = first_half[first_next] * absolute_rotation[first_last][second_axis] +
                                      first_half[first_last] * absolute_rotation[first_next][second_axis];
          const double second_radius = second_half[second_next] * absolute_rotation[first_axis][second_last] +
                                       second_half[second_last] * absolute_rotation[first_axis][second_next];
          const double projected_translation = std::abs(translation[first_last] * rotation[first_next][second_axis] -
  ...
  {
    return m_implementation->data ? static_cast<std::size_t>(m_implementation->data->GetNumberOfPoints())
                                  : 0;
  }
  ...
  public:
    ...
    vtkSmartPointer<vtkIdTypeArray> candidate_ids = vtkSmartPointer<vtkIdTypeArray>::New();
    ...
    void Build_Self_Collision_Pairs()
    {
      ...
      for (std::size_t first = 0; first < parts.size(); ++first)
      {
        for (std::size_t second = first + 1; second < parts.size(); ++second)
        {
          ...
          {
            enabled = std::find(scene_options.self_collision_pairs.begin(),
                                scene_options.self_collision_pairs.end(),
                                std::array<std::size_t, 2>{first_source, second_source}) != scene_options.self_collision_pairs.end();
          }
          if (!enabled)
            continue;
          ...
          pair.detector = vtkSmartPointer<vtkCollisionDetectionFilter>::New();
  ...
  Robot_Collision_Detector::Robot_Collision_Detector(Robot_Collision_Detector &&) noexcept = default;
  Robot_Collision_Detector &Robot_Collision_Detector::operator=(Robot_Collision_Detector &&) noexcept = default;

  bool Is_Robot_Collision_Recovery_Improvement(const Robot_Collision_Result &original_collision,
                                               const Robot_Collision_Result &candidate_collision,
                                               double previous_clearance_margin_mm)
  {
    if (!original_collision.collided ||
        !candidate_collision.collided ||
        original_collision.type != candidate_collision.type ||
        original_collision.robot_part_index != candidate_collision.robot_part_index)
    {
      return false;
    }
    if (original_collision.type == Robot_Collision_Type::Self_Collision &&
        original_collision.other_robot_part_index != candidate_collision.other_robot_part_index)
    {
      return false;
    }
    return candidate_collision.clearance_margin_mm > previous_clearance_margin_mm + 1.0e-6;
  }

  void Robot_Collision_Detector::Set_Robot_Parts(const std::vector<Robot_Visual_Part> &parts)
  {
    Set_Robot_Parts_Internal(parts, true);
  }

  void Robot_Collision_Detector::Set_Robot_Parts_For_Obstacle_Filtering(const std::vector<Robot_Visual_Part> &parts)
  {
    Set_Robot_Parts_Internal(parts, false);
  }

  void Robot_Collision_Detector::Set_Robot_Parts_Internal(const std::vector<Robot_Visual_Part> &parts,
                                                          bool build_pose_query_geometry)
  {
    ...
    for (std::size_t source_index = 0; source_index < parts.size(); ++source_index)
    {
      ...
      part.signed_distance = vtkSmartPointer<vtkImplicitPolyDataDistance>::New();
      ...
      if (build_pose_query_geometry)
      {
        ...
        {
          sample_voxels.reserve(static_cast<std::size_t>(mesh_points->GetNumberOfPoints()));
          for (vtkIdType point_id = 0; point_id < mesh_points->GetNumberOfPoints(); ++point_id)
          {
            double point[3] = {};
  ...
  void Robot_Collision_Detector::Set_Scene_Collision_Options(const Robot_Scene_Collision_Options &options)
  {
    m_implementation->scene_options = options;
    m_implementation->Build_Self_Collision_Pairs();
  }

  const Robot_Scene_Collision_Options &Robot_Collision_Detector::Scene_Collision_Options() const
  {
    return m_implementation->scene_options;
  }

  bool Robot_Collision_Detector::Set_Obstacle_Points(const std::vector<float> &xyz, std::string *error_message)
  {
    return Set_Obstacle_Points(xyz, Robot_Collision_Point_Cloud_Options{},
                               {}, nullptr,
                               error_message);
  }

  bool Robot_Collision_Detector::Set_Obstacle_Points(const std::vector<float> &xyz,
                                                     ...
                                                     const std::atomic_bool *cancel_requested)
  {
    ...
    if (options.exclude_robot_points)
    {
      ...
      {
        if (part.source_index >= reference_world_from_parts.size())
          continue;
        const auto &world_from_part = reference_world_from_parts[part.source_index];
        reference_parts.push_back({&part,
    ...
    for (std::size_t index = 0; index < xyz.size() / 3; ++index)
    {
      ...
      {
        if (error_message)
          *error_message = "Collision obstacle build cancelled";
        if (stats)
          *stats = local_stats;
        return false;
      }
      const Point3 point = {xyz[index * 3], xyz[index * 3 + 1], xyz[index * 3 + 2]};
      if (!finite_point(point) ||
          !occupied_voxels.insert(voxel_key(point, options.voxel_size_mm)).second)
      {
        continue;
      }
      ...
      {
        if (!point_inside_inflated_bounds(point,
                                          reference.world_bounds,
                                          options.robot_exclusion_distance_mm))
          continue;
        const auto local_point = Transform_Position(reference.part_from_world, point);
        double local_query[3] = {local_point[0], local_point[1], local_point[2]};
        const double signed_distance = reference.part->signed_distance->EvaluateFunction(local_query);
        if (signed_distance < 0.0 ||
            std::abs(signed_distance) <= options.robot_exclusion_distance_mm)
        {
    ...
    if (points->GetNumberOfPoints() <= 0)
    {
      if (local_stats.voxel_point_count > 0 &&
          local_stats.excluded_robot_point_count == local_stats.voxel_point_count)
      {
      ...
      if (error_message)
        *error_message = "Obstacle point array has no finite points";
      return false;
    }
    ...
    local_stats.collision_point_count = static_cast<std::size_t>(points->GetNumberOfPoints());
    if (stats)
      *stats = local_stats;
    return true;
  }
  ...
  void Robot_Collision_Detector::Set_Obstacle_Snapshot(std::shared_ptr<const Point_Cloud_Collision_Snapshot> snapshot)
  {
    m_implementation->obstacle_snapshot = std::move(snapshot);
  }

  std::shared_ptr<const Point_Cloud_Collision_Snapshot> Robot_Collision_Detector::Obstacle_Snapshot() const
  {
  ...
  {
    return m_implementation->obstacle_snapshot ? m_implementation->obstacle_snapshot->Point_Count()
                                               : 0;
  }

  const Robot_Collision_Query_Stats &Robot_Collision_Detector::Last_Query_Stats() const
  {
    return m_implementation->last_query_stats;
  }

  Robot_Collision_Result Robot_Collision_Detector::Check_Pose(const std::vector<Matrix4> &world_from_parts,
                                                              double clearance_mm) const
  {
    m_implementation->last_query_stats = {};
    m_implementation->last_query_stats.minimum_self_sample_distance_mm = std::numeric_limits<double>::infinity();
    Robot_Collision_Result result;
    result.minimum_distance_mm = std::numeric_limits<double>::infinity();
    if (clearance_mm < 0.0 ||
        !std::isfinite(clearance_mm) ||
        !Has_Robot_Geometry())
    {
      return result;
    }
    ...
    std::vector<Pose_Part_Cache> pose_cache(m_implementation->parts.size());
    for (std::size_t part_index = 0; part_index < m_implementation->parts.size(); ++part_index)
    {
      const auto &part = m_implementation->parts[part_index];
      if (part.source_index >= world_from_parts.size())
        continue;
      auto &cache = pose_cache[part_index];
      cache.part_from_world = Invert_Rigid_Matrix(world_from_parts[part.source_index]);
      cache.world_bounds = transformed_bounds(part.local_bounds, world_from_parts[part.source_index]);
      cache.valid = true;
    }

    const auto &scene_options = m_implementation->scene_options;
    if (scene_options.check_ground_collision && std::isfinite(scene_options.ground_height_mm))
    {
      for (std::size_t part_index = 0; part_index < m_implementation->parts.size(); ++part_index)
      {
        const auto &part = m_implementation->parts[part_index];
        const auto &cache = pose_cache[part_index];
        const bool ground_checked = scene_options.ground_checked_parts.empty() ? part.source_index >= scene_options.first_ground_checked_part_index
                                                                               : std::find(scene_options.ground_checked_parts.begin(),
                                                                                           scene_options.ground_checked_parts.end(),
                                                                                           part.source_index) != scene_options.ground_checked_parts.end();
        if (!ground_checked || !cache.valid)
          continue;
        const auto &world_from_part = world_from_parts[part.source_index];
        if (cache.world_bounds[4] > scene_options.ground_height_mm + clearance_mm)
          continue;

        Point3 lowest_world = {0.0, 0.0, std::numeric_limits<double>::infinity()};
        for (const auto &local : part.surface_samples)
        {
          ++m_implementation->last_query_stats.ground_sample_queries;
          const auto world = Transform_Position(world_from_part, local);
          if (world[2] < lowest_world[2])
            lowest_world = world;
        }
        const double distance = lowest_world[2] - scene_options.ground_height_mm;
        if (distance <= clearance_mm)
        {
          ...
          result.obstacle_point_world = {lowest_world[0], lowest_world[1], scene_options.ground_height_mm};
          return result;
        }
      }
    }
    ...
    if (scene_options.check_self_collision)
    {
      const double self_clearance = std::max(0.0, scene_options.self_collision_clearance_mm);
      for (auto &pair : m_implementation->self_collision_pairs)
      {
        ...
        if (!first_cache.valid || !second_cache.valid)
          continue;
        ...
        if (!bounds_overlap(first_cache.world_bounds,
                            second_cache.world_bounds,
                            self_clearance))
        {
                                         ...
                                         bool source_is_first)
        {
          for (const auto &source_local : source_part.surface_samples)
          {
            ...
            if (!point_inside_inflated_bounds(source_world,
                                              target_cache.world_bounds,
                                              self_clearance))
              continue;
            ++m_implementation->last_query_stats.self_distance_sample_queries;
            const auto target_local = Transform_Position(target_cache.part_from_world, source_world);
            double query[3] = {target_local[0], target_local[1], target_local[2]};
            ...
            if (!target_part.surface_locator->FindClosestPointWithinRadius(query,
                                                                           search_radius,
                                                                           closest_target_local,
                                                                           closest_cell_id,
                                                                           closest_sub_id,
                                                                           distance_squared))
            {
            ...
            if (distance <= self_clearance)
            {
              ...
              signed_distance = target_part.signed_distance->EvaluateFunctionAndGetClosestPoint(query, signed_closest);
            }
            ...
            m_implementation->last_query_stats.minimum_self_sample_distance_mm = std::min(m_implementation->last_query_stats.minimum_self_sample_distance_mm, distance);
            if (distance >= minimum_distance)
              continue;
            minimum_distance = distance;
            const auto target_world = Transform_Position(world_from_target,
                                                         {closest_target_local[0], closest_target_local[1], closest_target_local[2]});
            if (source_is_first)
            {
        ...
        sample_distance(first, first_transform, second, second_transform, second_cache, true);
        sample_distance(second, second_transform, first, first_transform, first_cache, false);

        const bool proximity_collision = has_negative_distance || minimum_distance <= self_clearance;
        bool surfaces_intersect = false;
        if (!proximity_collision && bounds_overlap(first_cache.world_bounds, second_cache.world_bounds))
        {
          if (oriented_bounds_overlap(first.local_bounds, first_transform,
        ...
        if (!surfaces_intersect && !proximity_collision)
          continue;
        ...
        result.minimum_distance_mm = std::isfinite(minimum_distance) ? minimum_distance
                                                                     : 0.0;
        result.clearance_margin_mm = has_negative_distance ? most_negative_distance - self_clearance
        ...
        if (surfaces_intersect && !has_negative_distance && result.clearance_margin_mm >= 0.0)
        {
          result.clearance_margin_mm = -std::max(self_clearance, 0.001);
        }
        if (surfaces_intersect)
        {
          if (auto *contacts = pair.detector->GetContactsOutput(); contacts && contacts->GetNumberOfPoints() > 0)
          {
            double contact[3] = {};
            contacts->GetPoint(0, contact);
            result.robot_closest_point_world = {contact[0], contact[1], contact[2]};
    ...
    for (std::size_t part_index = 0; part_index < m_implementation->parts.size(); ++part_index)
    {
      ...
      if (!cache.valid)
        continue;
      const auto &world_from_part = world_from_parts[part.source_index];
      double query_bounds[6] = {cache.world_bounds[0] - clearance_mm,
      ...
      candidate_ids->Reset();
      obstacle.locator->FindPointsInArea(query_bounds, candidate_ids, true);
      m_implementation->last_query_stats.obstacle_candidate_points += static_cast<std::size_t>(candidate_ids->GetNumberOfValues());

      for (vtkIdType candidate = 0; candidate < candidate_ids->GetNumberOfValues(); ++candidate)
      {
        ...
        const Point3 world_point = {raw_world_point[0], raw_world_point[1], raw_world_point[2]};
        const auto local_point = Transform_Position(cache.part_from_world, world_point);
        if (!point_inside_inflated_bounds(local_point, part.local_bounds, clearance_mm))
          continue;
        double closest_local[3] = {};
        double local_query[3] = {local_point[0], local_point[1], local_point[2]};
        ++m_implementation->last_query_stats.obstacle_distance_queries;
        const double signed_distance = part.signed_distance->EvaluateFunctionAndGetClosestPoint(local_query, closest_local);
        const double distance = std::abs(signed_distance);
        const bool candidate_collided = signed_distance < 0.0 || distance <= clearance_mm;
        if (candidate_collided)
        {
          ...
          result.robot_closest_point_world = Transform_Position(world_from_part,
