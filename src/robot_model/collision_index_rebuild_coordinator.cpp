...
namespace robot_model
{
  ...
  Collision_Index_Rebuild_Coordinator::Collision_Index_Rebuild_Coordinator()
      : m_publish_generation(std::make_shared<std::atomic<std::uint64_t>>(0))
  {
  }
  ...
  bool Collision_Index_Rebuild_Coordinator::Submit_Source(Collision_Index_Build_Request request,
                                                          Dispatch dispatch,
                                                          Completion completion)
  {
    ...
    return Submit_Prepared(std::move(request),
                           std::move(dispatch),
                           std::move(completion));
  }

  bool Collision_Index_Rebuild_Coordinator::Submit_Settings(Collision_Index_Build_Request request,
                                                            Dispatch dispatch,
                                                            Completion completion)
  {
    if (m_shutdown)
  ...
  bool Collision_Index_Rebuild_Coordinator::Submit_Prepared(Collision_Index_Build_Request request,
                                                            Dispatch dispatch,
                                                            Completion completion)
  {
    if (!dispatch || !completion || m_shutdown)
      return false;

    const auto publish_generation = ++(*m_publish_generation);
    const auto generation_token = m_publish_generation;
    const auto worker_generation = m_build_service.Submit(std::move(request),
                                                          [publish_generation,
                                                           generation_token, dispatch = std::move(dispatch),
                                                           completion = std::move(completion)](std::uint64_t, Collision_Index_Build_Result result) mutable
                                                          {
