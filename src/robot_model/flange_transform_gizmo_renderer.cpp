...
vtkSmartPointer<vtkActor> make_handle (
...
vtkSmartPointer<vtkActor> make_ring (
  double radius,
...
Flange_Transform_Gizmo_Renderer::Flange_Transform_Gizmo_Renderer (
  double axis_length_mm,
  double ring_radius_mm)
...
void Flange_Transform_Gizmo_Renderer::Set_World_From_Flange (
  const Matrix4& world_from_flange)
{
  ...
  for( int row = 0; row < 4; ++row )
  {
    ...
    {
      m_world_from_flange->SetElement (
        row, column, world_from_flange[row][column]);
    }
  }
  m_world_from_flange->Modified ( );
  Ensure_Actors ( );
  for( const auto& actor : m_actors ) actor->SetUserMatrix (m_world_from_flange);
}
  ...
  int rotation_axis)
{
  ...
  for( int axis = 0; axis < 3; ++axis )
  {
    ...
    ring_property->SetColor (
      ring_highlighted ? 1.0 : colors[axis][0],
...
void Flange_Transform_Gizmo_Renderer::Ensure_Actors ( )
{
  ...
  const std::array<std::array<double, 3>, 3> colors = {
    std::array<double, 3> { 1.0, 0.1, 0.1 },
  ...
  for( const auto& actor : m_actors )
  {
    ...
    if( m_world_from_flange ) actor->SetUserMatrix (m_world_from_flange);
  }
}
...
void Flange_Transform_Gizmo_Renderer::Add_Actors ( )
{
  ...
  for( const auto& actor : m_actors ) m_renderer->AddActor (actor);
}

void Flange_Transform_Gizmo_Renderer::Remove_Actors ( )
{
  if( !m_renderer ) return;
  for( const auto& actor : m_actors ) m_renderer->RemoveActor (actor);
}
