...
constexpr double kViewCubeArrowOrigin[3] =
{
  -kViewCubeHalfSize,
  -kViewCubeHalfSize,
  -kViewCubeHalfSize
...
constexpr std::array<View_Cube_Axis_Info, 3> kViewCubeAxes =
{{
  { "X", { 1.0, 0.0, 0.0 }, { 0.95, 0.16, 0.18 } },
  { "Y", { 0.0, 1.0, 0.0 }, { 0.10, 0.80, 0.24 } },
  { "Z", { 0.0, 0.0, 1.0 }, { 0.10, 0.45, 0.95 } }
}};

constexpr std::array<View_Cube_Face_Info, 6> kViewCubeFaces =
{{
  { View_Cube_Face::front, 1, -1.0, "L" },
...
bool face_label_position_and_normal (
  View_Cube_Face face,
  double position[3],
  double normal[3])
{
...
View_Cube_Face pick_view_cube_face (
  vtkRenderer* renderer,
  double x,
  double y)
{
  ...
  auto test_plane =
    [&] (int axis, double plane_position, View_Cube_Face face)
    {
...
void compute_scene_center_and_radius (
  vtkRenderer* renderer,
  double center[3],
  double& radius)
{
...
vtkSmartPointer<vtkActor> make_plane_actor (
...
vtkSmartPointer<vtkTextActor> make_text_label_actor (
  const char* text,
  const double color[3],
  int font_size = 9,
  double opacity = 0.80)
...
bool set_projected_label_position (
  vtkRenderer* renderer,
  vtkTextActor* actor,
  const double world[3])
{
...
vtkSmartPointer<vtkActor> make_arrow_actor (
  const double start[3],
  const double direction[3],
  double length,
  const double color[3])
{
  double normalized_direction[3] =
  {
    direction[0],
    direction[1],
    direction[2]
  ...
  double rotation_angle = vtkMath::DegreesFromRadians (
    std::acos (std::clamp (vtkMath::Dot (x_axis, normalized_direction),
                           -1.0,
                           1.0)));
...
bool set_camera_for_view_cube_face (
  vtkRenderer* renderer,
  View_Cube_Face face)
{
  switch( face )
...
void View_Cube_Controller::Setup (
  ...
  vtkRenderer* scene_renderer)
{
  ...
  m_view_cube_assembly->AddPart (
    make_plane_actor (p100, p000, p101, front_color));
  m_view_cube_assembly->AddPart (
    make_plane_actor (p010, p110, p011, back_color));
  m_view_cube_assembly->AddPart (
    make_plane_actor (p000, p010, p001, left_color));
  m_view_cube_assembly->AddPart (
    make_plane_actor (p110, p100, p111, right_color));
  m_view_cube_assembly->AddPart (
    make_plane_actor (p101, p001, p111, top_color));
  m_view_cube_assembly->AddPart (
    make_plane_actor (p000, p100, p010, bottom_color));

  for( const auto& axis_info : kViewCubeAxes )
  {
    m_view_cube_assembly->AddPart (
      make_arrow_actor (kViewCubeArrowOrigin,
                        axis_info.direction,
...
void View_Cube_Controller::Update_Camera ( )
{
  ...
  double direction[3] =
  {
    scene_position[0] - scene_focal_point[0],
  ...
  for( std::size_t i = 0; i < kViewCubeFaces.size ( ); ++i )
  {
    ...
    const bool visible =
      face_label_position_and_normal (kViewCubeFaces[i].face, world, normal) &&
