...
Flange_Drag_Motion_Execution Flange_Drag_Motion_Executor::Execute (
  ...
  const Pose_Mover& move_pose) const
{
  ...
  {
    if( !move_position ) return execution;
  ...
  execution.pose_changed = execution.apply_result.state_changed ||
    execution.apply_result.scene_changed;
