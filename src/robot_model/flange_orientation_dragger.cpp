...
Matrix4 rotate_orientation_world (
  const Matrix4& start,
  const Point3& axis,
  double angle)
{
  ...
  double bx, double by)
{
  ...
  const double amount = length_squared > 1.0e-12
    ? std::clamp ((( px - ax ) * dx + ( py - ay ) * dy) /
...
Flange_Orientation_Dragger::Flange_Orientation_Dragger (
...
bool Flange_Orientation_Dragger::Begin_Drag (
  double display_x,
  double display_y,
  const Matrix4& world_from_flange)
{
  ...
  const double axis_length = std::sqrt (
    m_rotation_axis_world[0] * m_rotation_axis_world[0] +
    m_rotation_axis_world[1] * m_rotation_axis_world[1] +
    m_rotation_axis_world[2] * m_rotation_axis_world[2]);
  if( axis_length < 1.0e-12 ) return false;
  for( double& value : m_rotation_axis_world ) value /= axis_length;
  m_start_pointer_angle = std::atan2 (
    display_y - m_origin_display[1], display_x - m_origin_display[0]);
  m_selected_axis = best_axis;
  m_dragging = true;
  return true;
}

int Flange_Orientation_Dragger::Hit_Test (
  double display_x,
  double display_y,
  ...
  for( int axis = 0; axis < 3; ++axis )
  {
    ...
    for( int segment = 0; segment <= segment_count; ++segment )
    {
      ...
      if( !Display_From_World (
            Transform_Position (world_from_flange, local_point),
            &current_display) )
      {
        has_previous = false;
        continue;
      ...
      if( has_previous )
      {
        const double distance_squared = point_segment_distance_squared (
          display_x, display_y,
...
bool Flange_Orientation_Dragger::Update_Drag (
  double display_x,
  double display_y,
  Matrix4* target_world_from_flange) const
{
  ...
  const double current_angle = std::atan2 (
    display_y - m_origin_display[1], display_x - m_origin_display[0]);
  *target_world_from_flange = rotate_orientation_world (
    m_start_world_from_flange,
    m_rotation_axis_world,
...
bool Flange_Orientation_Dragger::Display_From_World (
  const Point3& world,
