...
{
  Matrix4 matrix = { };
  for( std::size_t index = 0; index < 4; ++index ) matrix[index][index] = 1.0;
  return matrix;
}
  ...
  const Robot_Part_Calibration& calibration)
{
  return Multiply_Matrices (
    Multiply_Matrices (
      Multiply_Matrices (
        rotation_x (calibration.rotate_xyz[0]),
...
Point3 Transform_Part_Point (const Robot_Part_Calibration& calibration,
                             const Point3& point)
{
  return Transform_Position (Build_Part_Calibration_Matrix (calibration),
                             point);
}
