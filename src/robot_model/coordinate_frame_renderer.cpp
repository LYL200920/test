...
vtkSmartPointer<vtkActor> make_axis (
...
Coordinate_Frame_Renderer::Coordinate_Frame_Renderer (
...
{
  Ensure_Actors ( );
  if( !m_origin_actor ) return;
  m_origin_actor->GetProperty ( )->SetColor (
    highlighted ? 1.0 : 1.0,
...
void Coordinate_Frame_Renderer::Set_World_From_Frame (
  const Matrix4& world_from_frame)
{
  ...
  for( int row = 0; row < 4; ++row )
  {
    ...
    {
      m_world_from_frame->SetElement (
        row, column, world_from_frame[row][column]);
    }
  }
...
void Coordinate_Frame_Renderer::Ensure_Actors ( )
{
  ...
  if( m_show_axes )
  {
    m_actors.push_back (make_axis (
      { m_axis_length_mm, 0.0, 0.0 }, { 1.0, 0.1, 0.1 }));
    m_actors.push_back (make_axis (
      { 0.0, m_axis_length_mm, 0.0 }, { 0.1, 1.0, 0.1 }));
    m_actors.push_back (make_axis (
      { 0.0, 0.0, m_axis_length_mm }, { 0.1, 0.35, 1.0 }));
  }
