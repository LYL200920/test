...
Flange_Drag_Update_Coordinator::Flange_Drag_Update_Coordinator (
  std::chrono::milliseconds minimum_interval)
  : m_minimum_interval (std::max (minimum_interval,
...
Flange_Drag_Schedule Flange_Drag_Update_Coordinator::Queue_At (
  int display_x,
  int display_y,
  Clock::time_point now)
{
  ...
  const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds> (
    now - m_last_processed_at);
  const auto remaining = m_minimum_interval - elapsed;
  return { false, static_cast<int> (
    std::max<std::int64_t> (1, remaining.count ( ))) };
}

void Flange_Drag_Update_Coordinator::Set_Final (
  int display_x,
  int display_y)
{
  if( !m_active ) return;
...
bool Flange_Drag_Update_Coordinator::Take_Pending (
  Flange_Drag_Pointer* pointer)
{
  if( !pointer || !m_active || !m_has_pending ) return false;
...
void Flange_Drag_Update_Coordinator::Mark_Processed_At (
  Clock::time_point now)
{
  if( m_active ) m_last_processed_at = now;
}
