...
void Robot_Collision_Service::Set_Robot_Parts (
...
void Robot_Collision_Service::Set_Scene_Options (
...
bool Robot_Collision_Service::Set_Obstacle_Source (
  ...
  std::string* error_message)
{
  ...
  return Rebuild_Obstacle_Points (
    reference_world_from_parts, error_message);
}

bool Robot_Collision_Service::Rebuild_Obstacle_Source (
  const Robot_Joint_State& reference_joint_state,
  const std::vector<Matrix4>& reference_world_from_parts,
  std::string* error_message)
{
...
bool Robot_Collision_Service::Set_Settings (
  const Robot_Collision_Settings& settings,
  const std::vector<Matrix4>& reference_world_from_parts,
  std::string* error_message)
{
  if( !Validate_Settings (settings, error_message) ) return false;
  const auto previous = m_settings;
  m_settings = settings;
  if( !m_enabled ) return true;
  if( !Rebuild_Obstacle_Points (
        reference_world_from_parts, error_message) )
  {
    m_settings = previous;
...
bool Robot_Collision_Service::Create_Points_Rebuild_Request (
  std::shared_ptr<const std::vector<float>> xyz,
  const Robot_Joint_State& reference_joint_state,
  const std::vector<Matrix4>& reference_world_from_parts,
  ...
  std::string* error_message) const
{
  ...
  if( !Validate_Settings (m_settings, error_message) ) return false;
...
bool Robot_Collision_Service::Create_Settings_Rebuild_Request (
  const Robot_Collision_Settings& settings,
  const std::vector<Matrix4>& reference_world_from_parts,
  Collision_Index_Build_Request* request,
  std::string* error_message) const
{
  if( error_message ) error_message->clear ( );
  if( !request || !Validate_Settings (settings, error_message) ) return false;
...
void Robot_Collision_Service::Apply_Rebuild_Result (
  Collision_Index_Build_Result result)
{
  if( !m_enabled || !result.success ) return;
...
Robot_Motion_Collision_Result Robot_Collision_Service::Evaluate_Motion (
  const Robot_Forward_Kinematics_Model& model,
  const Robot_Motion_Collision_Request& request)
{
  return m_motion_guard.Evaluate (
    model, m_robot_parts, m_detector, request);
}

const Robot_Collision_Result& Robot_Collision_Service::Check_Pose (
  const std::vector<Matrix4>& world_from_parts)
{
  m_current_collision = { };
  if( m_enabled && m_detector.Has_Robot_Geometry ( ) )
  {
    m_current_collision = m_detector.Check_Pose (
      world_from_parts, m_settings.clearance_mm);
  }
  return m_current_collision;
}

bool Robot_Collision_Service::Validate_Settings (
  const Robot_Collision_Settings& settings,
  std::string* error_message)
{
...
bool Robot_Collision_Service::Rebuild_Obstacle_Points (
  const std::vector<Matrix4>& reference_world_from_parts,
  std::string* error_message)
{
  if( error_message ) error_message->clear ( );
  ...
  return m_detector.Set_Obstacle_Points (
    *m_source_xyz,
