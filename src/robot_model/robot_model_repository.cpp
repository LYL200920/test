                       ...
                       const std::filesystem::path& rhs)
{
  ...
  const auto left_is_number =
    !left.empty ( ) &&
    std::all_of (left.begin ( ), left.end ( ),
                 [] (unsigned char c) { return std::isdigit (c); });
  const auto right_is_number =
    !right.empty ( ) &&
...
Robot_Model_Info describe_robot_model (const std::filesystem::path& dir)
{
  ...
  if( root )
  {
    const auto name = item_value_as_string (
      root, "ManipulatorModel", info.display_name.c_str ( ));
    if( !name.empty ( ) )
    {
  ...
  std::sort (info.stl_files.begin ( ), info.stl_files.end ( ),
             natural_stl_less);

  if( root )
  {
    std::vector<std::filesystem::path> xml_stl_files;
    const auto model_number = item_value_as_int (
      root, "ModelNumber", static_cast<int> (info.stl_files.size ( )));
...
bool contains_loadable_robot_model (
