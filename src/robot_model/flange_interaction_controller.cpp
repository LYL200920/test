...
void Flange_Interaction_Controller::Set_Mode (Flange_Interaction_Mode mode)
{
  End_Drag ( );
  m_mode = mode;
  m_position_dragger.Set_Enabled (
    mode == Flange_Interaction_Mode::Free_Translation);
  m_axis_translation_dragger.Set_Enabled (
    mode == Flange_Interaction_Mode::Transform_6D);
  m_orientation_dragger.Set_Enabled (
    mode == Flange_Interaction_Mode::Transform_6D);
  Clear_Hover ( );
  Update_Visibility ( );
}
...
void Flange_Interaction_Controller::Update_Flange_Pose (
  ...
  Flange_Drag_Update* update) const
{
  ...
  *update = { };
  ...
  {
    Point3 target = { };
    const bool updated = m_axis_translation_dragger.Is_Dragging ( )
      ? m_axis_translation_dragger.Update_Drag (display_x, display_y, &target)
  ...
  {
    Matrix4 target = { };
    if( !m_orientation_dragger.Update_Drag (
          display_x, display_y, &target) ) return false;
...
bool Flange_Interaction_Controller::Update_Hover (
  double display_x,
  double display_y,
  const Matrix4* world_from_flange,
  bool* changed)
{
  ...
  if( world_from_flange )
  {
    ...
    else if( m_mode == Flange_Interaction_Mode::Free_Translation )
    {
      ...
      free_translation = m_position_dragger.Hit_Test (
        display_x, display_y, origin);
    }
  }

  const bool hover_changed =
    translation_axis != m_hover_translation_axis ||
  ...
  if( hover_changed )
  {
    ...
    m_gizmo_renderer.Set_Highlighted_Handle (
      translation_axis, rotation_axis);
    m_free_drag_handle_renderer.Set_Origin_Highlighted (free_translation);
  }
  if( changed ) *changed = hover_changed;
  return translation_axis >= 0 || rotation_axis >= 0 || free_translation;
}

void Flange_Interaction_Controller::Update_Visibility ( )
{
  m_flange_frame_renderer.Set_Visible (
    m_frame_visible && m_has_flange_pose);
  m_gizmo_renderer.Set_Visible (
    m_mode == Flange_Interaction_Mode::Transform_6D && m_has_flange_pose);
  m_free_drag_handle_renderer.Set_Visible (
    m_mode == Flange_Interaction_Mode::Free_Translation &&
    m_has_flange_pose);
}
