...
namespace robot_model
{
  ...
  Flange_Axis_Translation_Dragger::Flange_Axis_Translation_Dragger(double axis_length_mm)
  ...
  bool Flange_Axis_Translation_Dragger::Begin_Drag(double display_x,
                                                   double display_y,
                                                   const Matrix4 &world_from_flange)
  {
    if (!m_enabled || !m_renderer)
      return false;
    m_start_origin_world = {world_from_flange[0][3],
                            world_from_flange[1][3],
                            world_from_flange[2][3]};
    Point3 origin_display = {};
    if (!Display_From_World(m_start_origin_world, &origin_display))
      return false;
    ...
    if (!Display_From_World(Transform_Position(world_from_flange, endpoint_local), &best_endpoint_display))
      return false;

    m_projected_axis_display = {best_endpoint_display[0] - origin_display[0],
                                best_endpoint_display[1] - origin_display[1],
                                0.0};
    m_projected_axis_length_squared = m_projected_axis_display[0] * m_projected_axis_display[0] +
                                      m_projected_axis_display[1] * m_projected_axis_display[1];
    if (m_projected_axis_length_squared < 20.0 * 20.0)
      return false;
    m_axis_world = {world_from_flange[0][best_axis],
                    world_from_flange[1][best_axis],
                    world_from_flange[2][best_axis]};
    const double length = std::sqrt(m_axis_world[0] * m_axis_world[0] +
                                    m_axis_world[1] * m_axis_world[1] +
                                    m_axis_world[2] * m_axis_world[2]);
    if (length < 1.0e-12)
      return false;
    for (double &value : m_axis_world)
      value /= length;
  ...
  int Flange_Axis_Translation_Dragger::Hit_Test(double display_x,
                                                double display_y,
                                                const Matrix4 &world_from_flange) const
  {
    ...
    for (int axis = 0; axis < 3; ++axis)
    {
      ...
      if (!Display_From_World(Transform_Position(world_from_flange, endpoint_local), &endpoint_display))
        continue;
  ...
  bool Flange_Axis_Translation_Dragger::Update_Drag(double display_x,
                                                    double display_y,
                                                    Point3 *target_position_world) const
  {
    ...
    const double distance_world = (delta_x * m_projected_axis_display[0] +
                                   delta_y * m_projected_axis_display[1]) /
                                  m_projected_axis_length_squared * m_axis_length_mm;
    for (int axis = 0; axis < 3; ++axis)
    {
      (*target_position_world)[axis] = m_start_origin_world[axis] + m_axis_world[axis] * distance_world;
    }
    return true;
  }
  ...
  bool Flange_Axis_Translation_Dragger::Display_From_World(const Point3 &world, Point3 *display) const
  {
