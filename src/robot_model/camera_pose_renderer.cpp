...
namespace robot_model
{
  namespace
  {
    ...
    vtkSmartPointer<vtkActor> make_line_actor(const std::array<double, 3> &start,
  ...
  void Camera_Pose_Renderer::Set_World_From_Camera(const Matrix4 &world_from_camera)
  {
    ...
    for (int row = 0; row < 4; ++row)
    {
      ...
      {
        m_world_from_camera->SetElement(row, column, world_from_camera[row][column]);
      }
    }
  ...
  void Camera_Pose_Renderer::Ensure_Actors()
  {
    ...
    m_actors.push_back(make_line_actor({0.0, 0.0, 0.0},
                                       {axis_length, 0.0, 0.0},
                                       {1.0, 0.1, 0.1}, 4.0));
    m_actors.push_back(make_line_actor({0.0, 0.0, 0.0},
                                       {0.0, axis_length, 0.0},
    ...
    const std::array<std::array<double, 3>, 4> corners = {std::array<double, 3>{-half_width, -half_height, depth},
    ...
    {
      m_actors.push_back(make_line_actor(origin, corners[i], frustum_color, 2.0));
      m_actors.push_back(make_line_actor(corners[i], corners[(i + 1) % corners.size()], frustum_color, 2.0));
    }
