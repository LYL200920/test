...
namespace robot_model
{
  namespace
  {
    ...
    template <std::size_t Task_Size>
    using Task_Jacobian = std::array<std::array<double, JOINT_COUNT>, Task_Size>;

    class Solve_Clock
    {
    public:
      explicit Solve_Clock(double time_budget_ms) : m_time_budget_ms(time_budget_ms),
      ...
      {
        return std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - m_started_at).count();
      }
    ...
    template <std::size_t Task_Size>
    bool solve_damped_least_squares(const Task_Jacobian<Task_Size> &jacobian,
                                    const Task_Vector<Task_Size> &error,
                                    double damping,
                                    Joint_Vector *joint_delta_radians)
    {
      if (!joint_delta_radians)
        return false;
      ...
      if (!solve_linear_system(normal, error, &weighted_error))
        return false;
      ...
      for (std::size_t joint = 0; joint < JOINT_COUNT; ++joint)
      {
        for (std::size_t row = 0; row < Task_Size; ++row)
        {
          (*joint_delta_radians)[joint] += jacobian[row][joint] * weighted_error[row];
        }
      }
      return true;
    }

    double lower_limit_at(const Robot_Kinematic_Params &params, std::size_t index)
    {
      return index < params.joint_mins.size() ? params.joint_mins[index]
                                              : -std::numeric_limits<double>::infinity();
    }

    double upper_limit_at(const Robot_Kinematic_Params &params, std::size_t index)
    {
      return index < params.joint_maxs.size() ? params.joint_maxs[index]
                                              : std::numeric_limits<double>::infinity();
    }
    ...
    bool joint_is_locked(const Robot_Kinematic_Params &params, std::size_t joint)
    {
      return std::abs(upper_limit_at(params, joint) - lower_limit_at(params, joint)) < 1.0e-12;
    }

    Joint_Vector clamped_input_angles(const Robot_Joint_State &initial_state, const Robot_Kinematic_Params &params)
    {
                           ...
                           Joint_Vector *input_angles)
    {
      ...
      for (std::size_t joint = 0; joint < JOINT_COUNT; ++joint)
      {
        const double delta_degrees = std::clamp(delta_radians[joint] * RADIANS_TO_DEGREES,
                                                -max_step,
                                                max_step);
        (*input_angles)[joint] = clamp_joint_input((*input_angles)[joint] + delta_degrees, params, joint);
      }
    }

    Task_Jacobian<3> build_position_jacobian(const Robot_Forward_Kinematics_Result &forward,
                                             const Robot_Kinematic_Params &params,
                                             const Point3 &flange_position)
    {
      ...
      for (std::size_t joint = 0; joint < JOINT_COUNT; ++joint)
      {
        if (joint_is_locked(params, joint))
          continue;
        const auto radius = subtract(flange_position, forward.joint_positions_world[joint]);
        const auto derivative = cross_product(forward.joint_axes_world[joint], radius);
    ...
    Task_Jacobian<6> build_pose_jacobian(const Robot_Forward_Kinematics_Result &forward,
                                         const Robot_Kinematic_Params &params,
                                         const Point3 &flange_position,
                                         double orientation_weight)
    {
      Task_Jacobian<6> jacobian = {};
      const auto position_jacobian = build_position_jacobian(forward, params, flange_position);
      for (std::size_t joint = 0; joint < JOINT_COUNT; ++joint)
      {
        for (std::size_t row = 0; row < 3; ++row)
        {
          ...
          if (!joint_is_locked(params, joint))
          {
            jacobian[row + 3][joint] = forward.joint_axes_world[joint][row] * Joint_Direction_At(params, joint) * orientation_weight;
          }
        }
      }
      return jacobian;
    ...
    Point3 orientation_error_vector(const Matrix4 &target, const Matrix4 &current)
    {
      double relative[3][3] = {};
      for (std::size_t row = 0; row < 3; ++row)
      {
        for (std::size_t column = 0; column < 3; ++column)
        {
          ...
          {
            relative[row][column] += target[row][item] * current[column][item];
          }
        }
      }

      const double cosine = std::clamp((relative[0][0] + relative[1][1] + relative[2][2] - 1.0) * 0.5,
                                       -1.0,
                                       1.0);
      const double angle = std::acos(cosine);
      if (angle < 1.0e-9)
      {
        return {(relative[2][1] - relative[1][2]) * 0.5,
      ...
      {
        Point3 axis = {std::sqrt(std::max(0.0, (relative[0][0] + 1.0) * 0.5)),
                       std::sqrt(std::max(0.0, (relative[1][1] + 1.0) * 0.5)),
                       std::sqrt(std::max(0.0, (relative[2][2] + 1.0) * 0.5))};
        if (relative[2][1] - relative[1][2] < 0.0)
          axis[0] = -axis[0];
        if (relative[0][2] - relative[2][0] < 0.0)
          axis[1] = -axis[1];
        if (relative[1][0] - relative[0][1] < 0.0)
          axis[2] = -axis[2];
        const double length = vector_length(axis);
        if (length > 1.0e-12)
        {
          for (double &value : axis)
            value = value * angle / length;
        }
        return axis;
      }

      const double scale = angle / (2.0 * sine);
      return {(relative[2][1] - relative[1][2]) * scale,
  ...
  Robot_Position_IK_Result Solve_Flange_Position_IK(const Robot_Forward_Kinematics_Model &model,
                                                    const Robot_Kinematic_Params &params,
                                                    ...
                                                    const Robot_Position_IK_Options &options)
  {
    ...
    for (std::size_t iteration = 0; iteration <= options.max_iterations; ++iteration)
    {
      ...
      const auto jacobian = build_position_jacobian(forward, params, current_position);
      Joint_Vector joint_delta = {};
      if (!solve_damped_least_squares(jacobian, error, damping, &joint_delta))
        break;
      apply_joint_delta(joint_delta, params, options.max_joint_step_deg, &input_angles);
    }
  ...
  Robot_Pose_IK_Result Solve_Flange_Pose_IK(const Robot_Forward_Kinematics_Model &model,
                                            const Robot_Kinematic_Params &params,
                                            const Robot_Joint_State &initial_state,
                                            ...
                                            const Robot_Pose_IK_Options &options)
  {
    ...
    auto input_angles = clamped_input_angles(initial_state, params);
    const double position_tolerance = std::max(options.position_tolerance_mm, 0.0);
    const double orientation_tolerance_radians = std::max(options.orientation_tolerance_deg, 0.0) * PI / 180.0;
    const double orientation_weight = std::max(options.orientation_weight_mm, 1.0e-6);
    const double damping = std::max(options.damping_mm, 1.0e-6);
    ...
    for (std::size_t iteration = 0; iteration <= options.max_iterations; ++iteration)
    {
      ...
      const auto orientation_error = orientation_error_vector(target_world_from_flange, current);
      ...
      const auto jacobian = build_pose_jacobian(forward, params, current_position, orientation_weight);
      const Task_Vector<6> error = {position_error[0], position_error[1], position_error[2],
