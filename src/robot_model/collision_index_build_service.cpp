...
namespace robot_model
{
  ...
  Collision_Index_Build_Result Build_Collision_Index_Snapshot(Collision_Index_Build_Request request, const std::atomic_bool *cancel_requested)
  {
    ...
    const auto finish = [&]
    {
      result.stats.build_time_ms = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - started_at).count();
    };
    ...
    result.success = builder.Set_Obstacle_Points(*result.source_xyz, result.settings.point_cloud,
                                                 request.reference_world_from_parts, &result.stats,
                                                 &result.error_message,
                                                 cancel_requested);
    if (result.success)
      result.obstacle_snapshot = builder.Obstacle_Snapshot();
    result.cancelled = cancel_requested && cancel_requested->load(std::memory_order_relaxed);
    finish();
    return result;
  }
  ...
  std::uint64_t Collision_Index_Build_Service::Submit(Collision_Index_Build_Request request,
                                                      Completion completion)
  {
    ...
    auto cancel_requested = std::make_shared<std::atomic_bool>(false);
    {
      ...
      if (m_stopping)
        return 0;
      Cancel_Locked();
      m_pending = Task{generation, std::move(request),
  ...
  void Collision_Index_Build_Service::Worker_Loop()
  {
    for (;;)
    {
      ...
      auto result = Build_Collision_Index_Snapshot(std::move(task.request), task.cancel_requested.get());
      Completion completion;
      {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (m_active_cancel == task.cancel_requested)
          m_active_cancel.reset();
        if (m_stopping || result.cancelled || task.generation != m_generation.load())
        {
          continue;
        }
        m_busy.store(false);
      ...
      if (completion)
        completion(task.generation, std::move(result));
    }
  }
