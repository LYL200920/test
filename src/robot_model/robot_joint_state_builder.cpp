...
{
  return index < params.joint_offsets.size ( )
    ? params.joint_offsets[index]
    : 0.0;
}
...
Robot_Joint_State Build_Joint_State_From_Input_Angles (
  ...
  const std::array<double, 6>& input_angles_deg)
{
  ...
  for( size_t i = 0; i < input_angles_deg.size ( ); ++i )
  {
    ...
    const double effective_angle =
      direction * input_angle + Joint_Offset_At (params, i);
    const double delta_angle =
      direction * ( input_angle - Neutral_Joint_Input_At (params, i) );
