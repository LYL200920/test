...
namespace robot_model
{
  ...
  void Robot_Drag_Performance_Collector::Record(const Robot_Drag_Performance_Sample &sample)
  {
    if (!m_active)
      return;
    ...
    m_stats.obstacle_candidate_points += sample.collision_query_stats.obstacle_candidate_points;
    m_stats.obstacle_distance_queries += sample.collision_query_stats.obstacle_distance_queries;
    m_stats.self_exact_pair_queries += sample.collision_query_stats.self_exact_pair_queries;
    m_stats.total_update_time_ms += sample.update_time_ms;
    m_stats.maximum_update_time_ms = std::max(m_stats.maximum_update_time_ms, sample.update_time_ms);
  ...
  bool Robot_Drag_Performance_Collector::Finish(Robot_Drag_Performance_Stats *stats)
  {
    if (!m_active)
      return false;
    m_active = false;
