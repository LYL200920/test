...
Robot_Motion_Collision_Result Robot_Motion_Collision_Guard::Evaluate (
  const Robot_Forward_Kinematics_Model& forward_model,
  ...
  const Robot_Motion_Collision_Request& request) const
{
  ...
  const auto start_transforms = Compute_Forward_Kinematics (
    forward_model, request.current_state);
  ...
  auto target_transforms = Compute_Forward_Kinematics (
    forward_model, target_state);
  auto influence_radii = collision_influence_radii (
    robot_parts, start_transforms, target_transforms);
  ...
  for( int limiting_pass = 0;
       request.options.maximum_sweep_pose_count > 0 &&
         step_count > request.options.maximum_sweep_pose_count &&
         limiting_pass < 8;
       ++limiting_pass )
  {
    const double fraction = static_cast<double> (
      request.options.maximum_sweep_pose_count) /
      static_cast<double> (step_count);
    target_state = Interpolate_Robot_Joint_State (
      request.current_state, target_state, fraction);
    ...
    const std::vector<Matrix4>& world_from_parts)
  {
    ...
    auto collision = collision_detector.Check_Pose (
      world_from_parts, request.options.clearance_mm);
    result.collision_query_time_ms +=
      std::chrono::duration<double, std::milli> (
        std::chrono::steady_clock::now ( ) - started_at).count ( );
    accumulate_query_stats (
      collision_detector.Last_Query_Stats ( ),
  ...
  for( std::size_t step = 1; step <= step_count; ++step )
  {
    ...
    result.blocking_collision = check_collision (
      sample_transforms.world_from_parts);
    if( result.blocking_collision.collided )
    {
      const bool improves_current_collision = recovering &&
        Is_Robot_Collision_Recovery_Improvement (
          request.current_collision, result.blocking_collision,
          recovery_margin);
