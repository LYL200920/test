  ...
  for( std::size_t joint = 0; joint < state.input_angles_deg.size ( ); ++joint )
  {
    ...
    state.effective_angles_deg[joint] = start.effective_angles_deg[joint] +
      ( target.effective_angles_deg[joint] -
        start.effective_angles_deg[joint] ) * t;
...
{
  if( maximum_joint_step_deg <= 0.0 ||
      !std::isfinite (maximum_joint_step_deg) )
  {
    return 1;
  }
...
{
  const auto angular_steps = Calculate_Robot_Joint_Sweep_Step_Count (
    start, target, maximum_joint_step_deg);
  if( maximum_spatial_step_mm <= 0.0 ||
      !std::isfinite (maximum_spatial_step_mm) )
  {
  ...
  {
    const double delta_radians = std::abs (
      target.delta_angles_deg[joint] - start.delta_angles_deg[joint]) *
      degrees_to_radians;
    const double radius = std::max (0.0, joint_influence_radii_mm[joint]);
    motion_bound_mm += delta_radians * radius;
  }
  const auto spatial_steps = std::max<std::size_t> (
    1, static_cast<std::size_t> (
      std::ceil (motion_bound_mm / maximum_spatial_step_mm)));
