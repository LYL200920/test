...
namespace robot_model
{
  ...
  bool Flange_Position_Dragger::Begin_Drag(double display_x,
                                           double display_y,
                                           const Point3 &flange_position_world)
  {
    ...
    if (!World_From_Display(display_x, display_y, flange_display[2], &pointer_world))
    {
      return false;
    }
    for (std::size_t axis = 0; axis < 3; ++axis)
    {
      m_pointer_to_flange_offset_world[axis] = flange_position_world[axis] - pointer_world[axis];
    }
  ...
  bool Flange_Position_Dragger::Hit_Test(double display_x,
                                         double display_y,
  ...
  bool Flange_Position_Dragger::Update_Drag(double display_x,
                                            double display_y,
                                            Point3 *target_position_world) const
  {
    ...
    Point3 pointer_world = {};
    if (!World_From_Display(display_x, display_y, m_drag_display_depth, &pointer_world))
    {
      return false;
    }
    for (std::size_t axis = 0; axis < 3; ++axis)
    {
      (*target_position_world)[axis] = pointer_world[axis] + m_pointer_to_flange_offset_world[axis];
    }
    return true;
  }
  ...
  bool Flange_Position_Dragger::Display_From_World(const Point3 &world, Point3 *display) const
  {
  ...
  bool Flange_Position_Dragger::World_From_Display(double display_x,
                                                   double display_y,
                                                   double display_z,
                                                   Point3 *world) const
  {
    ...
    *world = {homogeneous[0] / homogeneous[3],
              homogeneous[1] / homogeneous[3],
              homogeneous[2] / homogeneous[3]};
    return std::isfinite((*world)[0]) &&
