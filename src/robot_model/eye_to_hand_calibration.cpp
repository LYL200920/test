...
namespace robot_model
{
  namespace
  {
    ...
    bool values_for_key(const std::string &text,
                        std::string_view key,
                        ...
                        std::string *error_message)
    {
      ...
      {
        set_error(error_message, "Calibration key has no complete bracketed value: " + std::string(key));
        return false;
      }

      *values = extract_numbers(std::string_view(text).substr(open + 1, close - open - 1));
      if (values->size() != expected_count)
      {
        set_error(error_message, "Calibration key " + std::string(key) + " expected " +
      ...
      for (const double value : *values)
      {
        ...
        {
          set_error(error_message, "Calibration key contains a non-finite value: " + std::string(key));
          return false;
        }
      }
      return true;
  ...
  bool Load_Eye_To_Hand_Calibration(const std::filesystem::path &path,
                                    ...
                                    std::string *error_message)
  {
    ...
    if (!values_for_key(text, "RotMat_cam2robot", 9, &rotation, error_message) ||
        !values_for_key(text, "TranMat_cam2robot", 3, &translation, error_message))
    {
      return false;
    }
    ...
    if (text.find("CameraCoordinateType") != std::string::npos &&
        !values_for_key(text, "CameraCoordinateType", 1, &coordinate_type, error_message))
    {
      return false;
    }
    std::vector<double> point_cloud_unit;
    if (text.find("PointCloudUnitToMm") != std::string::npos &&
        !values_for_key(text, "PointCloudUnitToMm", 1, &point_cloud_unit, error_message))
    {
      return false;
    }
    ...
    for (int row = 0; row < 3; ++row)
    {
      for (int column = 0; column < 3; ++column)
      {
        parsed.camera_to_robot[row][column] = rotation[static_cast<std::size_t>(row * 3 + column)];
      }
    ...
    if (!coordinate_type.empty())
    {
      ...
      {
        set_error(error_message, "CameraCoordinateType must be 1 (Depth) or 2 (RGB)");
        return false;
      }
    ...
    {
      set_error(error_message, "RotMat_cam2robot is not a valid rotation matrix");
      return false;
    }
