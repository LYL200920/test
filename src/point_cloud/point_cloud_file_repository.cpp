...
namespace point_cloud
{
                                                                   ...
                                                                   std::uint32_t source_frame_number)
  {
    ...
    if (directory_error)
    {
      result.error_message = "Unable to create point-cloud directory: " + directory_error.message();
      return result;
    }
    ...
    metadata.saved_unix_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    if (!Save_Robot_Base_Point_Cloud_Ply(path, cloud, metadata, &result.error_message))
    {
      return result;
    }
    result.success = true;
  ...
  Point_Cloud_File_Save_Result Save_Robot_Base_Point_Cloud_To_Resource(const Point_Cloud_Data &cloud,
                                                                       const std::string &robot_model_id,
                                                                       std::uint32_t source_frame_number)
  {
    Point_Cloud_File_Save_Result result;
    ...
    if (directory_error)
    {
      result.error_message = "Unable to create Resource/PointCloud: " + directory_error.message();
      return result;
    }
    ...
    {
      path = resource_root / (base_name + "_" + std::to_string(suffix) + ".ply");
    }

    return Save_Robot_Base_Point_Cloud_To_File(path, cloud, robot_model_id, source_frame_number);
  }

  bool Load_Robot_Base_Point_Cloud_For_Model(const std::filesystem::path &path,
                                             const std::string &expected_robot_model,
                                             Point_Cloud_Data *cloud,
                                             Point_Cloud_File_Metadata *metadata,
                                             std::string *error_message)
  {
    if (!Load_Robot_Base_Point_Cloud_Ply(path, cloud, metadata, error_message))
    {
      return false;
    }
    if (!expected_robot_model.empty() && metadata->robot_model != expected_robot_model)
    {
      if (cloud)
        *cloud = {};
      if (error_message)
