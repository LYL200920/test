...
public:
            ...
            wxBORDER_SIMPLE | wxFULL_REPAINT_ON_RESIZE)
  {
    ...
    Bind(wxEVT_ERASE_BACKGROUND, &Point_Cloud_Canvas::On_Erase_Background, this);
  ...
  bool Load_Ply(const std::filesystem::path &path, std::string *error_message)
  {
    Ensure_VTK();
  ...
  bool Set_Live_Point_Cloud(const Camera_Point_Cloud &cloud, std::string *error_message)
  {
    Ensure_VTK();
    if (!m_scene || !m_scene->Is_Ready())
    {
...
Point_Cloud_View::Point_Cloud_View(wxWindow *parent, Camera_Service &camera_service)
    : wxPanel(parent, wxID_ANY),
      m_camera_service(camera_service)
{
  auto *title = new wxStaticText(this, wxID_ANY, From_Utf8(u8"点云显示"));
  m_live_button = new wxButton(this, wxID_ANY, From_Utf8(u8"实时相机"));
  auto *load_button = new wxButton(this, wxID_ANY, From_Utf8(u8"加载 PLY"));
  auto *clear_button = new wxButton(this, wxID_ANY, From_Utf8(u8"清空"));
  auto *fit_button = new wxButton(this, wxID_ANY, From_Utf8(u8"适应视图"));
  m_status_text = new wxStaticText(this, wxID_ANY, From_Utf8(u8"等待相机点云"));
  m_canvas = new Point_Cloud_Canvas(this);
  m_canvas->SetMinSize(wxSize(320, 240));

  m_live_button->Bind(wxEVT_BUTTON, &Point_Cloud_View::On_Live_Camera, this);
...
void Point_Cloud_View::Submit_Frame(std::shared_ptr<const Camera_Frame> frame)
{
...
void Point_Cloud_View::Worker_Loop()
{
  while (true)
  {
    Conversion_Job job;
    {
      ...
      m_worker_ready.wait(lock,
                          [this]
                          {
                            return m_exit_requested || m_pending_job.has_value();
                          });
      if (m_exit_requested)
      {
        return;
    ...
    result.success = Convert_Camera_Frame_To_Point_Cloud(*job.frame, &result.cloud, &result.error);
    {
      std::lock_guard<std::mutex> lock(m_worker_mutex);
...
void Point_Cloud_View::Consume_Result()
{
  ...
  if (m_source_mode != Source_Mode::Live_Camera || result->epoch != m_live_epoch)
  {
    return;
  }
  if (!result->success)
  {
    m_status_text->SetLabel(From_Utf8(u8"无法显示：") + From_Utf8(result->error));
    return;
  }
  ...
  {
    m_status_text->SetLabel(From_Utf8(u8"显示失败：") + From_Utf8(error));
    return;
  }
...
void Point_Cloud_View::On_Load_Ply(wxCommandEvent &)
{
  ...
  std::string error;
  if (!m_canvas->Load_Ply(std::filesystem::path(dialog.GetPath().ToStdWstring()), &error))
  {
    m_status_text->SetLabel(From_Utf8(u8"加载失败：") + From_Utf8(error));
    return;
  }

  m_source_mode = Source_Mode::Ply_File;
  ++m_live_epoch;
  m_status_text->SetLabel(wxString::Format(From_Utf8(u8"PLY 已加载，有效点=%zu"), m_canvas->Point_Count()));
}
...
void Point_Cloud_View::On_Clear(wxCommandEvent &)
{
  ...
  ++m_live_epoch;
  m_canvas->Clear();
  m_status_text->SetLabel(From_Utf8(u8"点云已清空；点击“实时相机”可继续显示"));
}
