...
constexpr const char* kApplicationMarker =
  "comment application robot_point_cloud_overlay";
...
bool Save_Robot_Base_Point_Cloud_Ply (
  ...
  std::string* error_message)
{
  ...
  if( cloud.xyz.empty ( ) || ( cloud.xyz.size ( ) % 3 ) != 0 ||
      cloud.Point_Count ( ) > kMaximumFilePoints )
  {
  ...
  if( metadata.coordinate_frame != "robot_base" ||
      metadata.unit != "millimeter" ||
      !is_safe_comment_value (metadata.robot_model) )
  {
  ...
  {
    std::filesystem::remove (temporary_path);
    set_error (error_message, "Unable to finalize point-cloud file: " +
...
bool Load_Robot_Base_Point_Cloud_Ply (
  const std::filesystem::path& path,
  ...
  std::string* error_message)
{
  ...
  *metadata = {};
  ...
  while( std::getline (file, line) )
  {
    ...
    if( line == "ply" ) saw_ply = true;
    else if( line == "format binary_little_endian 1.0" ) saw_format = true;
    else if( line == kApplicationMarker ) saw_application = true;
    ...
    else if( starts_with (line, "comment source_frame_number ") )
    {
      try
      {
        metadata->source_frame_number = static_cast<std::uint32_t> (
          std::stoul (comment_value (line, "source_frame_number")));
      }
      catch( ... )
      {
    ...
    else if( starts_with (line, "comment saved_unix_milliseconds ") )
    {
      try
      {
        metadata->saved_unix_milliseconds = std::stoll (
          comment_value (line, "saved_unix_milliseconds"));
      }
      catch( ... )
      {
    ...
    {
      ++property_count;
      if( line == "property uchar red" || line == "property uchar green" ||
          line == "property uchar blue" ) has_color = true;
    }
      ...
      metadata->unit != "millimeter" )
  {
    set_error (error_message,
      "PLY is not a robot-base/millimeter overlay file created by this application");
    return false;
  }
  const std::size_t expected_properties = has_color ? 6 : 3;
  if( vertex_count == 0 || vertex_count > kMaximumFilePoints ||
  ...
  if( has_color ) cloud->rgb.resize (vertex_count * 3);
  for( std::size_t index = 0; index < vertex_count; ++index )
  {
    file.read (reinterpret_cast<char*> (cloud->xyz.data ( ) + index * 3),
               sizeof (float) * 3);
    if( has_color )
    {
