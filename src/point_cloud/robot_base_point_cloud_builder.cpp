...
namespace point_cloud
{
                                    ...
                                    std::string *error_message)
  {
    if (error_message)
      error_message->clear();
    if (bounds == nullptr || cloud.xyz.empty() || (cloud.xyz.size() % 3) != 0)
    {
      if (error_message)
        *error_message = "Point cloud is empty or malformed";
      return false;
    }
    *bounds = {std::numeric_limits<double>::infinity(),
    ...
    for (std::size_t index = 0; index < cloud.Point_Count(); ++index)
    {
      for (int axis = 0; axis < 3; ++axis)
      {
        ...
        (*bounds)[axis * 2 + 1] = std::max((*bounds)[axis * 2 + 1], value);
      }
    }
    return true;
  }
  ...
  bool Are_Robot_Base_Bounds_Reasonable(const std::array<double, 6> &bounds)
  {
    ...
    {
      if (!std::isfinite(value) || std::abs(value) > kMaximumWorldMagnitudeMm)
      {
        return false;
      }
    }
                                    ...
                                    std::string *error_message)
  {
    ...
    {
      if (error_message)
        *error_message = "Robot-base point-cloud output is null";
      return false;
    }
    *output = {};
    ...
    if (!Convert_Camera_Frame_To_Point_Cloud(frame, &camera_cloud, error_message, 500000, calibration.camera_coordinate_type))
    {
      return false;
    }
    ...
    {
      const auto world = robot_model::Transform_Position(world_from_camera,
                                                         {camera_cloud.xyz[index * 3],
                                                          camera_cloud.xyz[index * 3 + 1],
    ...
    if (!Calculate_Point_Cloud_Bounds(output->data, &output->world_bounds_mm, error_message) || !Are_Robot_Base_Bounds_Reasonable(output->world_bounds_mm))
    {
