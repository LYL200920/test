...
wxString Format_Stream_Configuration (
  const std::vector<Camera_Stream_Config>& streams,
  bool loaded)
{
  if( !loaded )
  {
  ...
  wxString text = wxString::Format (
    From_Utf8 (u8"预计输出 %zu 路："), streams.size ( ));
  ...
  : wxScrolledWindow (parent, wxID_ANY)
{
  ...
  auto* title = new wxStaticText (
    this, wxID_ANY, From_Utf8 (u8"采集控制"));
  m_device_text = new wxStaticText (
    this, wxID_ANY, From_Utf8 (u8"设备：未打开"));
  m_state_text = new wxStaticText (
    this, wxID_ANY, From_Utf8 (u8"状态：未初始化"));
  m_stream_config_text = new wxStaticText (
    this, wxID_ANY, From_Utf8 (u8"尚未读取流配置"));
  m_frame_text = new wxStaticText (
    this, wxID_ANY, From_Utf8 (u8"帧数据：未采集"));

  m_refresh_streams_button = new wxButton (
    this, wxID_ANY, From_Utf8 (u8"刷新流配置"));
  m_start_button = new wxButton (
    this, wxID_ANY, From_Utf8 (u8"开始采集"));
  m_stop_button = new wxButton (
    this, wxID_ANY, From_Utf8 (u8"停止采集"));
  m_close_button = new wxButton (
    this, wxID_ANY, From_Utf8 (u8"关闭相机"));

  m_start_button->Bind (
    wxEVT_BUTTON, &Camera_Acquisition_Panel::On_Start, this);
  m_stop_button->Bind (
    wxEVT_BUTTON, &Camera_Acquisition_Panel::On_Stop, this);
  m_close_button->Bind (
    wxEVT_BUTTON, &Camera_Acquisition_Panel::On_Close, this);
  m_refresh_streams_button->Bind (
    wxEVT_BUTTON, &Camera_Acquisition_Panel::On_Refresh_Streams, this);
  ...
  auto* frame_sizer = new wxStaticBoxSizer (
    wxVERTICAL, this, From_Utf8 (u8"采集数据"));
  frame_sizer->Add (m_frame_text, 0, wxEXPAND | wxALL, 8);

  auto* stream_sizer = new wxStaticBoxSizer (
    wxVERTICAL, this, From_Utf8 (u8"预计输出流"));
  stream_sizer->Add (
    m_stream_config_text, 0, wxEXPAND | wxLEFT | wxRIGHT | wxTOP, 8);
  stream_sizer->Add (
    m_refresh_streams_button, 0, wxEXPAND | wxALL, 8);
  ...
  sizer->Add (
    acquisition_buttons, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 8);
           ...
           m_last_frame_time.time_since_epoch ( ).count ( ) != 0 )
  {
    ...
    if( seconds > 0.0 )
    {
      const auto sample = static_cast<double> (
        frame->generation - m_last_generation) / seconds;
